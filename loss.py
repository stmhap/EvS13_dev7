...
class YoloLoss(LightningModule):
    ...
    def forward(self, predictions, target, anchors):
        # return self.calculate_loss(predictions.to(self.device), target.to(self.device), anchors.to(self.device))
        return self.calculate_loss(predictions, target, anchors)


class YoloLossCumulative(LightningModule):
    def __init__(self):
    ...
    def forward(self, predictions, target, scaled_anchors):
        ...
        for i in range(len(target)):
            combined_loss += self.yolo_loss(predictions[i], target[i], scaled_anchors[i])
